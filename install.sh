...
APT_REPO_TMP="deb [arch=amd64 signed-by=/etc/apt/keyrings/sonaric.gpg] $APT_DOWNLOAD_URL sonaric-releases-tmp main"
      ...
      exec_cmd "DEBIAN_FRONTEND=noninteractive apt-get satisfy -y -qq --dry-run 'podman (>=3.4.0)' > $DEVNULL" || (
        ...
        exit 1
			...
			(
				...
				exec_cmd "apt-cache madison podman | grep -v 4.9.3 || echo \"$APT_REPO_TMP\" > /etc/apt/sources.list.d/sonaric_tmp.list"
				exec_cmd "apt-get update -qq > $DEVNULL"
			)
...
do_update() {
  print_message "Updating Sonaric..."
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
      exec_cmd "apt-get update -qq > $DEVNULL"
      exec_cmd "podman version | grep 4.9.3 && echo \"$APT_REPO_TMP\" > /etc/apt/sources.list.d/sonaric_tmp.list && apt-get update -qq > $DEVNULL && apt-get install podman > $DEVNULL"
